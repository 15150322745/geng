		...
		watch:{
			'checkoneid':{ //编辑区域这是qt部分发生编辑的获取
				handler(newvalue){
					...
					.addClass('my-group').addClass(_this.checkonejson.checkAreaId)
					.on('click',function(ev){
						...
						// console.log(this)
						// console.log(_this.checkonejson,'编辑')
						//发送btn2的宽高
						_this.btn2width = _this.checkonejson.boundingRect[2]
						_this.btn2height =_this.checkonejson.boundingRect[3]
						//先清除所有外边框
						...
						this.on('mouseup',function(){
							...
							//需要发送的数据
							var gx= parseFloat(this.attr().transform.substring(7).split(',')[4])
							var gy= parseFloat(this.attr().transform.substring(7).split(',')[5])
							...
							_this.changeJson.boundingRect.push(Number(_this.oX)+gx)
							_this.changeJson.boundingRect.push(Number(_this.oY)+gy)
				...
				handler(newvalue,oldvalue){
					...
					.on('click',function(ev){
						...
						this.on('mouseup',function(){
							...
							var obx = parseInt(newvalue.boundingRect[0])
							var oby = parseInt(newvalue.boundingRect[1])
							var obh = parseInt(newvalue.boundingRect[3])
							$("#d_btn").css({"left":obx+nbx,"top":oby+obh+nby})
						...
						for(let i = 0; i < newvalue.length; i++){
							...
							.on('click',function(ev){
								...
								this.on('mouseup',function(){
									...
									var obx = parseInt(newvalue[i].boundingRect[0])
									var oby = parseInt(newvalue[i].boundingRect[1])
									var obh = parseInt(newvalue[i].boundingRect[3])
									$("#d_btn").css({"left":obx+nbx,"top":oby+obh+nby})
							...
							// 	SVG.select('.my-group').selectize(false).resize('stop').draggable('stop',ev)
							// 	//添加单独一个边框
							// 	this.selectize(ev).selectize({deepSelect:true}).draggable(ev).resize(ev);
							// })
